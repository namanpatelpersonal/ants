    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                #if the insect that im trying to add is an ant, and theres already another ant in this place
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.backwards and not self.scared_before:
            destination = self.place.entrance
        # END EC
        if self.blocked():
            self.sting(self.place.ant)
        elif self.armor > 0 and destination is not None and destination is not colony.hive:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place is not hive:
            ...
            place, dist = place.entrance, dist + 1
        return None
    # YOUR CODE HERE
        '''def helper(place, count_places, min_range, max_range):
            if count_places == max_range:
                return None 
            if count_places >= min_range and count_places < max_range:
                if place is hive:
                    return None
                elif len(place.bees) > 0:
                    return random_or_none(place.bees)
                else:
                    return helper(place.entrance, count_places+1, min_range, max_range)
            else:
                return helper(place.entrance, count_places+1, min_range, max_range)
        return helper(self.place, count_places, self.min_range, self.max_range)'''
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_Imposter:
            ...
            super().action(colony)
            self.buff(self.place.exit)
        # END Problem 13

    def buff(self, place):
        if place: #if the place is an actual place
            if place.ant:
                if not place.ant.buffed: #if there is an ant in the place and its not buffed already
                    place.ant.damage *= 2
